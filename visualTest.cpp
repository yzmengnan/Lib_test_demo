...
public:
	...
	getMat(cv::Mat &srcImg)
	{
		...
		srcImg =
		    cv::Mat(frame.iHeight, frame.iWidth, CV_8UC3, (unsigned char *)pBuffer);
		if (NULL == srcImg.data) {
			return false;
		}
		return true;
...
//	this_thread::sleep_for(chrono::seconds(5));
	...
	auto angles_6axis =
	    vector<double>{angles_now.begin(), angles_now.begin() + 6};
	...
	if (argc >= 2)
	{
		...
		{
			case 0:
				delta_based_on_end[0] += 0.2;
				break;
			case 1:
				delta_based_on_end[1] += 0.2;
				break;
			case 2:
				delta_based_on_end[0] -= 0.2;
				break;

...
vector<vector<double>> _interpolation::_3_Order_CubicSpline_is_time_adjustment(vector<vector<double>> yData, const int& totalTime) {
...
//    cout<<"derltaBuffer"<<endl<<deltaBuffer<<endl;
    ...
    for (int i{}; i < deltaBuffer.size(); i++) {
        if (i >= 1)
            timeArray.push_back((int) (deltaBuffer[i] / base * trajectoryPointsCount + (int) timeArray[i])+1);
        else

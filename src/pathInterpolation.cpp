...
auto _interpolation::_1_Order_Linear_no_time_adjustment(const vector<vector<double>> &pathPointsData, const int &costTime) -> vector<vector<double>> {
...
vector<vector<double>> _interpolation::_3_Order_CubicSpline_is_time_adjustment(vector<vector<double>> yData, size_t totalTime) {
    ...
    auto trajectoryPointsCount{10 * totalTime};
    ...
    if (jointsCount != 0) {
        for (int i{}; i < pathPointsCount - 1; i++) {
            Eigen::VectorXd buffer = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(yData[i].data(), (long long) jointsCount);
            Eigen::VectorXd bufferNext = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(yData[i + 1].data(), (long long) jointsCount);
    ...
    for (int i{}; i < deltaBuffer.size(); i++) {
        if (i >= 1)
            timeArray.push_back((int) (deltaBuffer[i] / base * trajectoryPointsCount + (int) timeArray[i]));
        else

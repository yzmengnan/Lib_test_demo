...
{
	nErr = AdsSyncWriteReq(pAddr, OUTPUT_BASE, OUTPUT_OFFSET, DTS_SIZE * SendData.size(),
	                       SendData.data());
	if (nErr)
	{
...
{
	nErr =
	    AdsSyncReadReq(pAddr, INPUT_BASE, INPUT_OFFSET, DFS_SIZE * GetData.size(), GetData.data());
	if (nErr)
	{
...
{
	nErr = AdsSyncWriteReq(pAddr, OUTPUT_BASE, OUTPUT_OFFSET, DTG_SIZE_P * SendData.size(),
	                       SendData.data());
	if (nErr)
	{
		cout << "Error: Ads send error: " << nErr << endl;
...
{
	nErr = AdsSyncReadReq(pAddr, INPUT_BASE, INPUT_OFFSET, DFG_SIZE_P * GetData.size(),
	                      GetData.data());
	if (nErr)
	{
		//        cout<<"Error: Ads get error: "<<nErr<<endl;
...
{
	nErr = AdsSyncWriteReq(pAddr, OUTPUT_BASE, OUTPUT_OFFSET_GRAP_TORQUE,
	                       DTG_SIZE_T * SendData.size(), SendData.data());
	if (nErr)
	{
		cout << "Error: Ads send error: " << nErr << endl;
		return -101;
...
{
	nErr = AdsSyncReadReq(pAddr, INPUT_BASE, INPUT_OFFSET_GRAP_TORQUE, DFG_SIZE_T * GetData.size(),
	                      GetData.data());
	if (nErr)
	{
		//        cout<<"Error: Ads get error: "<<nErr<<endl;
...
void gt_ads::send()
{
	ads_send(OUTPUT_BASE, OUTPUT_OFFSET_GRAP_TORQUE, DTG_SIZE_T * Grap_Torque_Servo_Nums,
	         tx_data->data());
}
void gt_ads::receive()
{
	ads_receive(INPUT_BASE, INPUT_OFFSET_GRAP_TORQUE, DFG_SIZE_T * Grap_Torque_Servo_Nums,
	            rx_data->data());
}

...
driverSever::driverSever(const int& port, Tc_Ads& ads_handle)
    : MotionV1 {ads_handle}, comSocket {port}
{
		...
		while (true)
		{
			...
			if ((this->socketRecv->Command & 0b10) != 0)
			{
				...
				if ((this->socketRecv->Command & 0b100) != 0)
				{
					...
					{
						auto rate = abs(this->socketRecv->Cartesian_Velocity_set[0]) >= 0.01
						              ? this->socketRecv->Cartesian_Velocity_set[0]
						              : 1.0f;
					...
					else if (this->socketRecv->Command & 0x8000000)
					{
						...
						{
							rate = abs(this->socketRecv->Joint_Velocity_set[0]) >= 1
							         ? 1
							         : this->socketRecv->Joint_Velocity_set[0];
							this->setSyncrpm(rate * DEFAULT_SYNC_RPM);
							this->isSingleAxisMotion = true;
						}
						driver_errcode =
						    this->Write('3', singleJointNeedMoving[0], singleJointNeedMoving[1],
					...
					{
						auto rate = abs(this->socketRecv->Joint_Velocity_set[0]) >= 0.01
						              ? this->socketRecv->Joint_Velocity_set[0]
						              : 1.0f;
						this->setSyncrpm(rate * DEFAULT_SYNC_RPM);
						driver_errcode = this->Write('1', this->socketRecv->Joint_Position_set[0],
				...
				if ((this->socketRecv->Command & 0b1000) != 0)
				{
					...
					if (cspFirststep)
					{
						...
						auto jointDataTarget =
						    vec(vector<double> {angles.begin(), angles.begin() + 6});
						if (norm(jointDataNow - jointDataTarget) > 10)
						{
							cout << "Error! CSP start point is too far! Vector Norm is "
							     << norm(jointDataNow - jointDataTarget) << endl;
							cspFirststep = true;
							continue;
						}
					}
				...
				// CSP--offline
				{
					...
					if ((this->socketRecv->Command & 0b100000) != 0 && offline_pathPoints.size())
					{
						...
						offline_traj_data = _interpolation::_1_Order_Linear_no_time_adjustment(
						    offline_pathPoints, COST_TIME);
						//                        offline_traj_data =
						//                        _interpolation::_3_Order_CubicSpline_is_time_adjustment(offline_pathPoints,
						//                        COST_TIME);
						//                        //跑到初始位置
						auto jointDataNow = vec(MDT::getAngles(*this, this->MotGetData).data(), 6);
						auto jointDataFirstTarget = vec(vector<double> {
						    offline_pathPoints[0].begin(), offline_pathPoints[0].begin() + 6});
						if (norm(jointDataNow - jointDataFirstTarget) > 10)
						{
							cout << "running to the first path point!" << endl;
							// pp 1 csp 0 cst 0
							//                             this->servoOperationModeSet(true, false,
							//                             false);
							this->ppFlag   = true;
							this->cspFlag  = false;
							this->cst_Flag = false;
							this->setSyncrpm(50);
							driver_errcode =
							    this->Write('x', offline_pathPoints[0][0], offline_pathPoints[0][1],
							                offline_pathPoints[0][2], offline_pathPoints[0][3],
							                offline_pathPoints[0][4], offline_pathPoints[0][5],
							...
							// 通过上下使能，切换操作模式
							////                            this->servoOperationModeSet(false, true,
							/// false);
						}
						// 清空获取的路径点数据

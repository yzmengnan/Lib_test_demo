...
#define ADSIGRP_SUMUP_READ                                                                         \
	0xF080// AdsRW  IOffs list size or 0 (=0 -> list size == WLength/3*sizeof(ULONG))
...
#define ADSERR_DEVICE_SYMBOLNOTACTIVE                                                              \
	(0x22 + ERR_ADSERRS)// symbol not active -> release handle and try again
...
#define PADSSYMBOLTYPE(p)                                                                          \
	(((char*)(((PAdsSymbolEntry)p) + 1)) + ((PAdsSymbolEntry)p)->nameLength + 1)
#define PADSSYMBOLCOMMENT(p)                                                                       \
	(((char*)(((PAdsSymbolEntry)p) + 1)) + ((PAdsSymbolEntry)p)->nameLength + 1                    \
	 + ((PAdsSymbolEntry)p)->typeLength + 1)

#define PADSNEXTSYMBOLENTRY(pEntry)                                                                \
	(*((unsigned long*)(((char*)pEntry) + ((PAdsSymbolEntry)pEntry)->entryLength))                 \
	     ? ((PAdsSymbolEntry)(((char*)pEntry) + ((PAdsSymbolEntry)pEntry)->entryLength))           \
	     : NULL)
...
typedef struct {
	...
	// ADS_UINT8	copyMask[];		// "size" bytes containing 0xff or 0x00 - 0x00 means ignore byte
	// (ADSIGRP_SYM_VALBYHND_WITHMASK)
} AdsDatatypeEntry, *PAdsDatatypeEntry, **PPAdsDatatypeEntry;

#define PADSDATATYPENAME(p) ((PCHAR)(((PAdsDatatypeEntry)p) + 1))
#define PADSDATATYPETYPE(p)                                                                        \
	(((PCHAR)(((PAdsDatatypeEntry)p) + 1)) + ((PAdsDatatypeEntry)p)->nameLength + 1)
#define PADSDATATYPECOMMENT(p)                                                                     \
	(((PCHAR)(((PAdsDatatypeEntry)p) + 1)) + ((PAdsDatatypeEntry)p)->nameLength + 1                \
	 + ((PAdsDatatypeEntry)p)->typeLength + 1)
#define PADSDATATYPEARRAYINFO(p)                                                                   \
	(PAdsDatatypeArrayInfo)(                                                                       \
	    ((PCHAR)(((PAdsDatatypeEntry)p) + 1)) + ((PAdsDatatypeEntry)p)->nameLength + 1             \
	    + ((PAdsDatatypeEntry)p)->typeLength + 1 + ((PAdsDatatypeEntry)p)->commentLength + 1)
...
__inline PAdsDatatypeEntry AdsDatatypeStructItem(PAdsDatatypeEntry p, unsigned short iItem)
{
	...
	if (iItem >= p->subItems)
		return NULL;
	pItem =
	    (PAdsDatatypeEntry)(((unsigned char*)(p + 1)) + p->nameLength + p->typeLength
	                        + p->commentLength + 3 + p->arrayDim * sizeof(AdsDatatypeArrayInfo));
	for (i = 0; i < iItem; i++)

 ...
 *
 *@section 开发建议
 *-# 在@link #DVP_INFORMATION 可预料@endlink的情况下，请确保所有标准API返回@link
 *#dvpStatus::DVP_STATUS_OK
 *DVP_STATUS_OK@endlink（譬如，避免对黑白相机调用彩色相机才有的特性）。这有助于提高程序的健壮度
 *-# 善用<a href="C:/Program Files
 *(x86)/Do3think/BasedCam2">Basedcam2</a>官方演示程序的“开发者模式”。这有助于加深对每个API接口的功能和应用的理解
 *-# 最基础的API接口都安排在@link #DVP2_API_BASE
 *基本功能@endlink分组下。这有助于快速把握相机的核心功能
 ...
 *-# 增加了根据@link #dvpCameraInfo::UserID 用户命名@endlink来@link #dvpOpenByUserId
 *打开相机@endlink的功能
 ...
 *-# 新增了一组@link #dvpLine
 *相机引脚@endlink操作相关的接口，建议别再使用dvpInputIo和dvpOutputIo系列的接口
...
typedef enum dvpStreamEvent
{
	...
	/** @brief 启动一个专门的线程以@link #dvpGetFrame dvpGetFrame@endlink
	 *的方式获取图像，并调用其他用户处理过程
...
/** @brief 设备打开模式
 *@note 支持"或"运算，比如：@code dvpOpen(0, dvpOpenMode(OPEN_NORMAL | OPEN_DEBUG), &handle)
 *@endcode*/
 ...
 *@ingroup DVP_INFORMATION
 *@note
 *请根据功能描述来调用相应的API。譬如，不要对黑白相机使用彩色相机才有的特性，以免产生不必要的错误状态码
 */
 ...
 *@return 可预料的情况下，请确保返回@link #dvpStatus::DVP_STATUS_OK DVP_STATUS_OK@endlink
 *@ingroup DVP_TRIGGER
 *@note 须先@link #dvpSetTriggerSource 设置@endlink好@link
 *#dvpTriggerSource::TRIGGER_SOURCE_SOFTWARE 软件触发源@endlink
 */
 ...
 *@see dvpRestart
 *@see dvpIsHold
 *@note 在启动视频流之前就可以执行该动作，当视频流@link #dvpStart
 *启动（相当于打开总水闸）@endlink后，再执行@link #dvpRestart
 *恢复（相当于打开水龙头）@endlink视频流，就能立即输出图像数据
 */
 ...
 *@see dvpGetColorTemperature
 */
dvp2api dvpStatus dvpGetColorTemperatureDescr(dvpHandle handle,
                                              dvpIntDescr* pColorTemperatureDescr);
 ...
 *@see dvpGetSoftTriggerLoop
 */
dvp2api dvpStatus dvpGetSoftTriggerLoopDescr(dvpHandle handle,
                                             dvpDoubleDescr* pSoftTriggerLoopDescr);
 ...
 *@see dvpGetStreamPackInterval
 */
dvp2api dvpStatus dvpGetStreamPackIntervalDescr(dvpHandle handle,
                                                dvpIntDescr* pStreamPackIntervalDescr);
 ...
 *@see dvpSetAeTarget
 *@note 自动曝光算法通过调节@link #dvpGetExposure 曝光时间@endlink和@link #dvpGetAnalogGain
 *模拟增益@endlink所要达到的最终亮度
 */
 ...
 *@note
 *外部电信号触发的瞬间可能存在一定的电压波动，所以需要设定一个恰当的消抖时间，以确保信号达到稳定
 */
 ...
 *@see dvpGetTriggerJitterFilter
 */
dvp2api dvpStatus dvpGetTriggerJitterFilterDescr(dvpHandle handle,
                                                 dvpDoubleDescr* pTriggerJitterFilterDescr);
 ...
 *@see dvpGetFramesPerTrigger
 */
dvp2api dvpStatus dvpGetFramesPerTriggerDescr(dvpHandle handle,
                                              dvpIntDescr* pFramesPerTriggerDescr);
 ...
 *@ingroup DVP_COLOR
 *@see dvpSetAwbOperation
 *@note 在执行白平衡操作后，须自行@link #dvpSetRgbGain 设置RGB增益@endlink为1.0，并且@link
 *dvpSetColorTemperatureState 禁用色温效果@endlink才能得到纯正的白平衡
 */
 ...
 * @warning 此操作建议在@link #dvpStart 启动视频流@endlink前设置好
 */
dvp2api dvpStatus dvpGetLineTriggerConfig(dvpHandle handle,
                                          dvpLineTriggerConfig* pLineTriggerConfig);
 ...
 */
dvp2api dvpStatus dvpGetOutputIoFunction(dvpHandle handle, dvpOutputIo outputIo,
                                         dvpOutputIoFunction* pOutputIoFunction);
 ...
 */
dvp2api dvpStatus dvpSetOutputIoFunction(dvpHandle handle, dvpOutputIo outputIo,
                                         dvpOutputIoFunction OutputIoFunction);
 ...
 */
dvp2api dvpStatus dvpGetInputIoFunction(dvpHandle handle, dvpInputIo inputIo,
                                        dvpInputIoFunction* pInputIoFunction);
 ...
 */
dvp2api dvpStatus dvpSetInputIoFunction(dvpHandle handle, dvpInputIo inputIo,
                                        dvpInputIoFunction InputIoFunction);
 ...
 *@see dvpGetStreamFlowCtrlSelDetail
 */
dvp2api dvpStatus dvpGetStreamFlowCtrlSelDescr(dvpHandle handle,
                                               dvpSelectionDescr* pStreamFlowCtrlSelDescr);
 ...
 *@see dvpGetStreamFlowCtrlSelDescr
 */
dvp2api dvpStatus dvpGetStreamFlowCtrlSelDetail(dvpHandle handle, dvpUint32 StreamFlowCtrlSel,
                                                dvpSelection* pStreamFlowCtrlSelDetail);
 ...
 *@see dvpGetColorSolutionSelDetail
 */
dvp2api dvpStatus dvpGetColorSolutionSelDescr(dvpHandle handle,
                                              dvpSelectionDescr* pColorSolutionSelDescr);
 ...
 *@see dvpGetColorSolutionSelDescr
 */
dvp2api dvpStatus dvpGetColorSolutionSelDetail(dvpHandle handle, dvpUint32 ColorSolutionSel,
                                               dvpSelection* pColorSolutionSelDetail);
 ...
 *@see dvpGetBayerDecodeSelDetail
 */
dvp2api dvpStatus dvpGetBayerDecodeSelDescr(dvpHandle handle,
                                            dvpSelectionDescr* pBayerDecodeSelDescr);
 ...
 *@see dvpGetBayerDecodeSelDescr
 */
dvp2api dvpStatus dvpGetBayerDecodeSelDetail(dvpHandle handle, dvpUint32 BayerDecodeSel,
                                             dvpSelection* pBayerDecodeSelDetail);
 ...
 *@see dvpGetSourceFormatSelDetail
 */
dvp2api dvpStatus dvpGetSourceFormatSelDescr(dvpHandle handle,
                                             dvpSelectionDescr* pSourceFormatSelDescr);
 ...
 *@see dvpGetSourceFormatSelDescr
 */
dvp2api dvpStatus dvpGetSourceFormatSelDetail(dvpHandle handle, dvpUint32 SourceFormatSel,
                                              dvpFormatSelection* pSourceFormatSelDetail);
 ...
 *@see dvpGetTargetFormatSelDetail
 */
dvp2api dvpStatus dvpGetTargetFormatSelDescr(dvpHandle handle,
                                             dvpSelectionDescr* pTargetFormatSelDescr);
 ...
 *@see dvpGetTargetFormatSelDescr
 */
dvp2api dvpStatus dvpGetTargetFormatSelDetail(dvpHandle handle, dvpUint32 TargetFormatSel,
                                              dvpFormatSelection* pTargetFormatSelDetail);
 ...
 *@see dvpGetResolutionModeSelDetail
 */
dvp2api dvpStatus dvpGetResolutionModeSelDescr(dvpHandle handle,
                                               dvpSelectionDescr* pResolutionModeSelDescr);
 ...
 *@see dvpGetResolutionModeSelDescr
 */
dvp2api dvpStatus dvpGetResolutionModeSelDetail(dvpHandle handle, dvpUint32 ResolutionModeSel,
                                                dvpResolutionMode* pResolutionModeSelDetail);
 ...
 *@see dvpGetAeSchemeSelDescr
 */
dvp2api dvpStatus dvpGetAeSchemeSelDetail(dvpHandle handle, dvpUint32 AeSchemeSel,
                                          dvpSelection* pAeSchemeSelDetail);
 ...
 *@see dvpGetQuickRoiSelDescr
 */
dvp2api dvpStatus dvpGetQuickRoiSelDetail(dvpHandle handle, dvpUint32 QuickRoiSel,
                                          dvpQuickRoi* pQuickRoiSelDetail);
 ...
 *@see dvpGetPixelRateSelDescr
 */
dvp2api dvpStatus dvpGetPixelRateSelDetail(dvpHandle handle, dvpUint32 PixelRateSel,
                                           dvpSelection* pPixelRateSelDetail);
 ...
 *@see dvpGetPacketSizeSelDetail
 */
dvp2api dvpStatus dvpGetPacketSizeSelDescr(dvpHandle handle,
                                           dvpSelectionDescr* pPacketSizeSelDescr);
 ...
 *@see dvpGetPacketSizeSelDescr
 */
dvp2api dvpStatus dvpGetPacketSizeSelDetail(dvpHandle handle, dvpUint32 PacketSizeSel,
                                            dvpSelection* pPacketSizeSelDetail);
 ...
 *@see dvpGetAccelerateSelDetail
 */
dvp2api dvpStatus dvpGetAccelerateSelDescr(dvpHandle handle,
                                           dvpSelectionDescr* pAccelerateSelDescr);
 ...
 *@see dvpGetAccelerateSelDescr
 */
dvp2api dvpStatus dvpGetAccelerateSelDetail(dvpHandle handle, dvpUint32 AccelerateSel,
                                            dvpSelection* pAccelerateSelDetail);
 ...
 *@see dvpHold
 *@see dvpStop
 *@note 为重量级操作，可能会消耗较长的时间，若要实现快速重启，可执行@link #dvpRestart
 *dvpRestart@endlink，前提是已经执行了@link #dvpHold dvpHold@endlink
 */
 ...
 *@warning
 *由本函数获得的图像数据缓存，在下一次调用本函数时，将变得无效，因此，在多线程同时获得图像的应用中，需要做好线程同步或临界段保护
 */
dvp2api dvpStatus dvpGetFrame(dvpHandle handle, dvpFrame* pFrame, void** pBuffer,
                              dvpUint32 timeout);
 ...
 *由本函数获得的图像数据缓存，在下一次调用本函数时，将变得无效，因此，在多线程同时获得图像的应用中，需要做好线程同步或临界段保护
 */
dvp2api dvpStatus dvpGetFrameBuffer(dvpHandle handle, dvpFrameBuffer* pRaw, dvpFrameBuffer* pOut,
                                    dvpUint32 timeout);
 ...
 *@see dvpGetFrame
 *@note 须先@link #dvpStart 播放@endlink视频流，才能使用此功能
 *@warning 如果在@link #dvpStreamCallback 视频流回调函数@endlink中使用此功能，则只能注册为@link
 *#dvpStreamEvent::STREAM_EVENT_FRAME_THREAD STREAM_EVENT_FRAME_THREAD@endlink事件类型
 */
dvp2api dvpStatus dvpProcessImage(dvpHandle handle, const dvpFrame* pSourceFrame,
                                  const void* pSourceBuffer, dvpFrame* pTargetFrame,
                                  void* pTargetBuffer, dvpUint32 targetBufferSize,
 ...
 */
dvp2api dvpStatus dvpDebugReport(dvpHandle handle, dvpReportPart part, dvpReportLevel level,
                                 bool bForce, dvpStr text, dvpUint32 param);
 ...
 *@warning 切勿在回调函数中调用任何需要@link #dvpStop
 *停止视频流@endlink的函数。某些函数会隐性地自动停止视频流，同样应当避免
 *@{
 */
/** @brief 视频流回调函数
 *@param[in] handle 相机句柄
 ...
 *@note 可以在回调函数中，@link #dvpProcessImage 处理@endlink，@link #dvpDrawPicture
 *显示@endlink与@link #dvpSavePicture 存储@endlink图像
 *@warning 切勿在回调函数中，@link #dvpOpen 打开@endlink，@link #dvpClose 关闭@endlink，@link
 *#dvpStart 播放@endlink，@link #dvpStop 停止@endlink相机，以及@link #dvpGetFrame 抓取图像@endlink
 */
typedef dvpInt32 (*dvpStreamCallback)(dvpHandle handle, dvpStreamEvent event, void* pContext,
 ...
 *@see dvpUnregisterStreamCallback
 */
dvp2api dvpStatus dvpRegisterStreamCallback(dvpHandle handle, dvpStreamCallback proc,
                                            dvpStreamEvent event, void* pContext);
 ...
 *@param[in] event 回调函数的事件类型
 *@param[in] pContext 回调函数的用户指针
 *@note 须与@link #dvpRegisterStreamCallback
 *注册回调函数@endlink所使用的参数一致才能注销对应的回调函数
 */
dvp2api dvpStatus dvpUnregisterStreamCallback(dvpHandle handle, dvpStreamCallback proc,
                                              dvpStreamEvent event, void* pContext);
...
typedef dvpInt32 (*dvpEventCallback)(dvpHandle handle, dvpEvent event, void* pContext,
                                     dvpInt32 param, struct dvpVariant* pVariant);
 ...
 */
dvp2api dvpStatus dvpRegisterEventCallback(dvpHandle handle, dvpEventCallback proc, dvpEvent event,
                                           void* pContext);
 ...
 *@param[in] event 回调函数的事件类型
 *@param[in] pContext 回调函数的用户指针
 *@note 须与@link #dvpRegisterEventCallback
 *注册回调函数@endlink所使用的参数一致才能注销对应的回调函数
 */
dvp2api dvpStatus dvpUnregisterEventCallback(dvpHandle handle, dvpEventCallback proc,
                                             dvpEvent event, void* pContext);
 ...
 *@param[in] file 文件名或完整路径（图像文件的格式由文件的扩展名来决定，目前支持的图像格式包括：bmp,
 *jpeg, jpg, png, tiff, tif, gif, dat(纯图像数据)）
 *@param[in] quality 图像品质，仅对jpeg格式有效，并且取值范围是[1,100]
 *@see dvpGetFrame
 *@see dvpStreamCallback
 */
dvp2api dvpStatus dvpSavePicture(const dvpFrame* pFrame, const void* pBuffer, dvpStr file,
                                 dvpInt32 quality);
 ...
 *@param[in] filePath
 *文件名或完整路径（图像文件的格式由文件的扩展名来决定，目前支持的图像格式包括：bmp, jpeg, jpg, png,
 *tiff, tif, gif, dat(纯图像数据)）
 *@param[in] quality 图像品质，仅对jpeg格式有效，并且取值范围是[1,100]
 *@see dvpSavePicture
 */
dvp2api dvpStatus dvpCaptureFile(dvpHandle handle, dvpUint32 ResoulutionModeSel, dvpRegion* roi,
                                 dvpUint32 timeout, dvpStr filePath, dvpInt32 quality);

/**
 *@brief 将图像压缩转换到用户缓存
 *@param[in] pFrame 帧信息
 *@param[in] pSrcBuffer 图像数据的内存首地址（目前支持的数据格式包括：RAW, RGB24）
 *@param[in] format 表示图像格式的字符串（形式为“jpg”、
 *“png”，或“*.jpg”、“*.png”，目前支持的图像格式包括：bmp, jpeg, jpg, png, tiff, tif, gif,
 *dat(纯图像数据)）
 *@param[in] quality 图像品质，仅对jpeg格式有效，并且取值范围是[1,100]
 ...
 */
dvp2api dvpStatus dvpPrintPictureToBuffer(const dvpFrame* pFrame, const void* pSrcBuffer,
                                          dvpStr format, dvpInt32 quality, void* pDstBuffer,
                                          dvpUint32 uByteSize, dvpUint32* pValidByteSize);

#ifdef _WIN32
/**
 *@brief 在窗口上显示图像
 *@param[in] pFrame 帧信息
 *@param[in] pBuffer 图像数据的内存首地址（目前支持的数据格式包括：@link
 *#dvpImageFormat::FORMAT_MONO MONO@endlink，@link #dvpImageFormat::FORMAT_BAYER_BG BAYER@endlink，
 *@link #dvpImageFormat::FORMAT_BGR24 BGR24@endlink）
 ...
 */
dvp2api dvpStatus dvpDrawPicture(const dvpFrame* pFrame, const void* pBuffer, HWND hWnd,
                                 LPRECT sRect, LPRECT dRect);

/**
 *@brief 启动录像，创建视频文件，并获得视频编码器的句柄
 *@param[in] filePath
 *视频文件名或完整路径（视频文件的格式由文件的扩展名来决定，目前支持的视频格式包括：wmv, avi）
 ...
 *@warning 欧洲和韩国等地区所发行的WINDOWS操作系统没有内置WMV格式支持，可能需要自行到 <a
 *href="https://www.codecguide.com/windows_media_codecs.htm">微软公司网站</a>下载并安装相应版本的更新
 */
dvp2api dvpStatus dvpStartVideoRecord(dvpStr filePath, dvpUint32 width, dvpUint32 height,
                                      dvpInt32 quality, dvpRecordHandle* pHandle);

/**
 *@brief 检查视频编码器的句柄是否有效
 *@param[in] handle 视频编码器句柄
 *@param[out] pValid 有效状态
 *@note @link #dvpStartVideoRecord 启动录像@endlink成功则句柄有效，@link #dvpStopVideoRecord
 *停止录像@endlink成功则句柄失效
 */
 ...
 *@param[in] pFrame 帧信息
 *@param[in] pBuffer 图像数据的内存首地址（目前支持的数据格式包括：@link
 *#dvpImageFormat::FORMAT_BGR24 BGR24@endlink）
 *@see dvpGetFrame
 *@see dvpStartVideoRecord
 *@see dvpStopVideoRecord
 *@note @link #dvpStartVideoRecord 启动录像@endlink之前，请正确@link #dvpSetTargetFormat
 *设定目标图像格式@endlink
 */
dvp2api dvpStatus dvpVideoRecordWrite(dvpRecordHandle handle, const dvpFrame* pFrame,
                                      const void* pBuffer);
 ...
 *@warning 可能会消耗较长的等待时间
 */
dvp2api dvpStatus dvpWriteUserData(dvpHandle handle, dvpUint32 addr, const void* pBuffer,
                                   dvpUint32 size);
 ...
 *@warning 此函数为内部使用的函数，参数细节暂未公开，不建议使用
 */
dvp2api dvpStatus dvpConfigEx(dvpUint32 index, dvpUint32 command, dvpUint32 param,
                              const void* pData);
 ...
 */
dvp2api dvpStatus dvpGetIntValue(IN dvpHandle handle, IN const char* strKey,
                                 OUT int64_t* pCurIntValue, OUT dvpIntDescr* pstIntDescr);
 ...
 */
dvp2api dvpStatus dvpGetUint64Value(IN dvpHandle handle, IN const char* strKey,
                                    OUT uint64_t* pCurUint64Value,
                                    OUT dvpUint64Descr* pstUint64Descr);
 ...
 */
dvp2api dvpStatus dvpGetEnumValue(IN dvpHandle handle, IN const char* strKey,
                                  OUT unsigned int* pCurEnumValue,
                                  OUT unsigned int nszSupportValue[64],
                                  OUT unsigned int* pnSupportNum);
 ...
 *@param[in] nValue 想要设置的设备的属性值
 */
dvp2api dvpStatus dvpSetEnumValue(IN dvpHandle handle, IN const char* strKey,
 ...
 */
dvp2api dvpStatus dvpSetEnumValueByString(IN dvpHandle handle, IN const char* strKey,
 ...
 */
dvp2api dvpStatus dvpGetFloatValue(IN dvpHandle handle, IN const char* strKey,
                                   OUT float* pCurFloatValue, OUT dvpFloatDescr* pstFloatDescr);
 ...
 */
dvp2api dvpStatus dvpGetDoubleValue(IN dvpHandle handle, IN const char* strKey,
                                    OUT double* pCurDoubleValue,
                                    OUT dvpDoubleDescr* pstDoubleDescr);
 ...
 */
dvp2api dvpStatus dvpGetStringValue(IN dvpHandle handle, IN const char* strKey,
                                    OUT char* pstStringValue, IN const int i32StrValueSize);
 ...
 */
dvp2api dvpStatus dvpSetStringValue(IN dvpHandle handle, IN const char* strKey,
                                    IN const char* strValue);
 ...
 *@warning 此函数为内部使用的函数，参数细节暂未公开，不建议使用
 */
dvp2api dvpStatus dvpGetSelectionDescr(dvpHandle handle, dvpStr name,
                                       dvpSelectionDescr* pSelectionDescr);
 ...
 *@warning 此函数为内部使用的函数，参数细节暂未公开，不建议使用
 */
dvp2api dvpStatus dvpGetSelectionDetail(dvpHandle handle, dvpStr name, dvpUint32 index,
...
dvp2api dvpStatus dvpWriteGenICamMem(dvpHandle handle, dvpUint32 addr, const void* pData,
                                     dvpUint32 uSize);

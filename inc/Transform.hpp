...
#include "mat.hpp"
#include "math.h"
inline vector<float> joint2Position(const vector<float> &joint_data) {
    ...
    mat T05 = T04 * T45;
    mat T06 = T05 * T56;
    return vector<float>{T06.data[0][3], T06.data[1][3], T06.data[2][3], 0.0f, 0.0f, 0.0f};
}

inline mat jacob0(const vector<float> &joint_data) {
    //only used for 6-DOF
    if (joint_data.size() != 6) {
        return mat();
    }
...
inline mat jacobe(const vector<float> &joint_data) {
    //only used for 6-DOF
    if (joint_data.size() != 6) {
        return mat();
    }
